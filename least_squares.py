...
popt_measles, pcov_measles  = curve_fit(models.F,df[0].values,df[1].values,p0=np.ones(3),bounds=(np.zeros(3),np.inf * np.ones(3)))
popt_mumps, pcov_mumps      = curve_fit(models.F,df[0].values,df[2].values,p0=np.ones(3),bounds=(np.zeros(3),np.inf * np.ones(3)))
popt_rubella, pcov_rubella  = curve_fit(models.F,df[0].values,df[3].values,p0=np.ones(3),bounds=(np.zeros(3),np.inf * np.ones(3)))
